...
For a narrative shot state one purpose, one audience-attention change, one readable action, and one stable end-state change. For observational/essay work, a sustained observation and perceptual change may carry the unit without a new plot event. For a still or non-narrative study use a visible communication goal and stable visual state. If several spatial or causal changes occur, use the `action` route first.
