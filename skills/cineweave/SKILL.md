---
name: cineweave
description: Turn a creative idea into a stage-aware AIGC studio brief and composable workflow plan, including a human-in-the-loop Midjourney visual-exploration handoff, master-reference convergence, consistent asset creation, story, shots and production. Use as the all-in-one entry when a request spans multiple CineWeave domains; do not use it when the user explicitly invokes one specialist for a bounded task.
---
...
ActionSequenceSpec, ShotSpec, ImagePrompt, MidjourneyPromptPack, Storyboard,
...
- `studio`: use the visual-first, human-in-the-loop path. Start with a compact Midjourney prompt pack, pause while the user explores and selects master references, then resume through reference evidence, visual-bible locks, story/shot/storyboard planning, asset recipes and QA.
...
13. For `studio`, return the smallest next stage rather than pretending the entire project is resolved. The first stage may produce `CreativeBrief` and `WorkflowPlan` with a composed `$cineweave-prompt` `midjourney_compile` step; the plan must show where the user pauses, what they bring back and which specialist owns the next artifact.
...
16. Generate story, script, continuity, action coverage, shots, storyboard, image prompts and production recipes as separate owned contracts. The studio path composes them; it never flattens them into one “master prompt”.
...
In `studio` mode, the workflow's first prompt-owned step should be an explicit
`MidjourneyPromptPack` projection, followed by a human gate and a reference
ingest step; do not claim the pack was executed or that a master reference was
selected.
