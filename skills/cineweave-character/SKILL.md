...
- `appearance_state`: define one controlled appearance state without mutating identity. Structure post-makeup skin material, makeup, hair, costume silhouette/construction/layering/fit, palette, materials, pairing, accessories, condition and movement response. Use independently versioned hero and video-safe looks when their detail or movement constraints differ. Read `references/character-appearance.md`; when the look is staged for a shot, Director consumes the exact state through `../cineweave-director/references/appearance-styling-direction.md`. Return `../../packages/cineweave-contracts/schemas/character-appearance-state.schema.json`.
